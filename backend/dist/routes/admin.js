...
router.patch('/users/:userId', updateUserValidation, async (req, res) => {
    try {
    ...
    `);
        res.json({
            ...
            analytics: {
                ...
                usage: usageAnalytics.rows.map((row) => ({
                    date: row.date,
                    messageCount: parseInt(row.message_count) || 0,
                    conversationCount: parseInt(row.conversation_count) || 0,
                ...
                errorRates: errorRates.rows.map((row) => ({
                    date: row.date,
...
router.patch('/api-keys/:keyId/deactivate', async (req, res) => {
    try {
        ...
        const result = await query('UPDATE api_keys SET is_active = false WHERE id = $1 RETURNING id, name', [keyId]);
        if (result.rows.length === 0) {
            return res.status(404).json({
                success: false,
