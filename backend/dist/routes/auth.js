...
import { hashPassword, comparePassword, generateTokens, createSession, authenticate } from '../middleware/auth';
...
router.post('/register', authLimiter, registerValidation, async (req, res) => {
    try {
        ...
        // Create user
        const userId = uuidv4();
        await query(`INSERT INTO users (id, username, email, password_hash, role, created_at, updated_at) 
       VALUES ($1, $2, $3, $4, $5, NOW(), NOW())`, [userId, username, email, hashedPassword, 'free']);
        // Generate tokens
        const user = {
            id: userId,
            username,
...
router.post('/login', authLimiter, loginValidation, async (req, res) => {
    try {
        ...
        // Find user
        const userResult = await query('SELECT id, username, email, password_hash, role FROM users WHERE username = $1', [username]);
...
router.post('/refresh', async (req, res) => {
    try {
        ...
        const decoded = jwt.verify(refreshToken, 'fallback_secret');
        const userId = decoded.userId;
        // Find user
        const userResult = await query('SELECT id, username, email, role FROM users WHERE id = $1', [userId]);
        const user = userResult.rows[0];
        if (!user) {
            return res.status(401).json({
                success: false,
...
router.get('/profile', authenticate, async (req, res) => {
    try {
        ...
        const userResult = await query('SELECT id, username, email, role, preferences, created_at FROM users WHERE id = $1', [userId]);
        const user = userResult.rows[0];
        if (!user) {
...
router.patch('/preferences', authenticate, async (req, res) => {
    try {
        ...
        await query('UPDATE users SET preferences = $1, updated_at = NOW() WHERE id = $2', [JSON.stringify(preferences), userId]);
