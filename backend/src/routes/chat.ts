...
import { authenticate, AuthenticatedRequest, getUserTierLimits } from '../middleware/auth.js';
...
router.post('/conversations', authenticate as any, createConversationValidation, async (req: Request, res: Response) => {
  try {
    ...
    await query(
      `INSERT INTO conversations (id, user_id, title, settings, preset)
       VALUES ($1, $2, $3, $4, $5)`,
      [conversationId, user.id, title, JSON.stringify(settings), preset]
    );
    ...
    if (systemPrompt) {
      await query(
        `INSERT INTO messages (id, conversation_id, role, content, token_count)
         VALUES ($1, $2, $3, $4, $5)`,
        [uuidv4(), conversationId, 'system', systemPrompt, 0]
      );
    }
    ...
    logger.error('Create conversation error', { error: error.message, userId: (req as AuthenticatedRequest).user?.id });
    res.status(500).json({
      success: false,
...
router.post('/conversations/:conversationId/upload', 
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      for (const file of files) {
        try {
          ...
          const optimizedPath = path.join(
            ...
            `optimized_${path.basename(file.path, path.extname(file.path))}.webp`
          );
          ...
          const fileId = uuidv4();
          await query(
            `INSERT INTO file_uploads (id, conversation_id, user_id, filename, original_name, file_path, optimized_path, file_size, mime_type, metadata)
      ...
      logger.error('Upload error', { error: error.message, userId: (req as AuthenticatedRequest).user?.id });
      res.status(500).json({
        success: false,
...
router.post('/conversations/:conversationId/messages', 
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      // Verify conversation ownership
      const conversation = await query(
        'SELECT id, settings FROM conversations WHERE id = $1 AND user_id = $2',
        [conversationId, user.id]
      );
      ...
      const conversationSettings = (conversation.rows[0] as any).settings || {};
      ...
      await query(
        `INSERT INTO messages (id, conversation_id, role, content, token_count, model, attachments)
         VALUES ($1, $2, $3, $4, $5, $6, $7)`,
        [userMessageId, conversationId, 'user', JSON.stringify(messageData), 0, finalModel, JSON.stringify(attachments)]
      );
      ...
      await query(
        `INSERT INTO messages (id, conversation_id, role, content, token_count, model)
         VALUES ($1, $2, $3, $4, $5, $6)`,
        [assistantMessageId, conversationId, 'assistant', aiResponse.content, aiResponse.tokenCount, finalModel]
      );
      ...
      logger.error('Send message error', { error: error.message, userId: (req as AuthenticatedRequest).user?.id });
      res.status(500).json({
        success: false,
...
router.get('/conversations/:conversationId', authenticate as any, async (req: Request, res: Response) => {
  try {
    const user = (req as AuthenticatedRequest).user!;
    const { conversationId } = req.params;
    ...
    const conversationData = conversation.rows[0] as any;
    const totalMessages = parseInt((totalCount.rows[0] as any).count);
    ...
    res.json({
      ...
      messages: messages.rows.reverse().map((msg: any) => ({
        id: msg.id,
        role: msg.role,
    ...
    logger.error('Get conversation error', { error: error.message, userId: (req as AuthenticatedRequest).user?.id });
    res.status(500).json({
      success: false,
...
router.get('/conversations/:conversationId/export', authenticate as any, async (req: Request, res: Response) => {
  try {
    const user = (req as AuthenticatedRequest).user!;
    const { conversationId } = req.params;
    ...
    if (format === 'markdown') {
      ...
      for (const msg of messages.rows) {
        ...
        if (msgData.role === 'system') continue;
    ...
    logger.error('Export conversation error', { error: error.message, userId: (req as AuthenticatedRequest).user?.id });
    res.status(500).json({
      success: false,
...
async function simulateAIResponse(messages: any[], settings: any) {
