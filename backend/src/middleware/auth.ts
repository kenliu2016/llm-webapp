...
const query = async (_sql: string, _params: any[] = []) => {
  return { rows: [] };
};
...
export const authenticate = async (req: Request, res: Response, next: NextFunction): Promise<void> => {
  try {
    ...
    try {
      ...
      logger.warn('Invalid JWT token', { error: (jwtError as Error).message });
      res.status(401).json({
        success: false,
...
export const optionalAuth = async (req: AuthenticatedRequest, _res: Response, next: NextFunction): Promise<void> => {
  try {
    ...
    try {
      ...
      logger.debug('Optional auth failed', { error: (jwtError as Error).message });
    }
...
export const getUserTierLimits = (role: string) => {
  switch (role) {
    case 'admin':
